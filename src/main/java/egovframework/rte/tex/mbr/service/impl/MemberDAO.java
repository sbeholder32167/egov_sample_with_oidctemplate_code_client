...
public class MemberDAO extends EgovAbstractDAO {
	...
	/**
	 * OIDC 연동에만 사용되는 사용자 정보 추출 메서드.<br>
	 * ID만으로 사용자 정보를 가져오므로 매우 위험한 메서드이며, OIDC 외 다른 용도로 사용되어서는 안된다.<br>
	 * @author sbeholder6684
	 * @since 2026-07-05
	 * @param vo id만 있으면 된다.
	 * @return 사용자 정보.
	 */
	public MemberVO getMemberOidc(MemberVO vo) throws Exception {
		return (MemberVO) select("memberDAO.getMemberOidc", vo);
	}
	/**
	 * 권한 추출 메서드
	 * @author sbeholder6684
	...
	public List<EgovMap> getAuthority(MemberVO vo) throws Exception {
		return (List<EgovMap>) list("memberDAO.getAuthority", vo);
	}

	/**
	 * 코드 추출 메서드<br>본 예제에서는 Code 테이블에서 권한명을 관리한다.
	 * @author sbeholder6684
	 * @since 2026-07-05
	 * @param name 권한명(ROLE_USER...)
	 * @return MemberVO에서 사용되는 권한 코드
	 * @throws Exception
	 */
	public String getCode(final String name) throws Exception{
		return (String) select("memberDAO.getCode", name);
	}
}
